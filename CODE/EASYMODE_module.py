...
def load_easy_mode(canvas, load_normal_mode=None):
    # Store image references
    canvas.image_refs = {}

    # Game setup
    matches = ["Fewa", "Fewa", "Computer", "Computer", "Pupcat", "Pupcat", "Teresa", "Teresa"]
    ...
    try:
        back_img = Image.open("cards/BACK.png").resize((168, 235))
    ...
    for name in set(matches):
        try:
            img = Image.open(f"cards/{name.lower()}.png").resize((168, 235))
    ...
    def flip_card(card_id, value):
        if front_images.get(value):
            ...
            canvas.create_text((x1 + x2) // 2, (y1 + y2) // 2, text=value, fill="black", font=("Helvetica", 16))
    ...
    def check_match():
        ...
        if v1 == v2:
            ...
            if len(matched) == len(matches):
                ...
                show_end_screen(canvas, digit_images, remaining_time, "easy")
        else:
    ...
    # Card positions
    positions = [(270, 218), (490, 218), (710, 218), (930, 218),
                 (270, 480), (490, 480), (710, 480), (930, 480)]

    card_buttons = []
    for i, (x, y) in enumerate(positions):
        card_id = canvas.create_image(x, y, image=canvas.image_refs['back'], anchor=tk.NW)
        rect_id = canvas.create_rectangle(x, y, x + 168, y + 235, fill='', outline='')
    ...
    def on_play_again_click(event=None):
        close_popup()
        load_easy_mode(canvas, load_normal_mode)

    def on_next_level_click(event=None):
        close_popup()
        if load_normal_mode:
            load_normal_mode(canvas)
    ...
    def open_settings_popup(event=None):
        ...
        try:
            ...
            end_btn = canvas.create_image(364, 450, image=end_photo)
        ...
        try:
            ...
            play_again_btn = canvas.create_image(670, 450, image=play_again_photo)
    ...
    try:
        title_img1 = Image.open("label/flipskoTitle.png")
        canvas.image_refs['title'] = ImageTk.PhotoImage(title_img1)
        flipsko_title = canvas.create_image(683, 90, image=canvas.image_refs['title'], anchor=tk.CENTER, tags="flipsko_title")
    except Exception as e:
        print("Title image failed:", e)

    try:
        label_img = Image.open("label/easyL.png").resize((237, 98))
        canvas.image_refs['easy_label'] = ImageTk.PhotoImage(label_img)
        canvas.create_image(1200, 90, image=canvas.image_refs['easy_label'], anchor=tk.CENTER)
    except Exception as e:
        print("Easy label failed:", e)
