...
def load_normal_mode(canvas,  load_difficult_mode=None):
    # Store image references
    canvas.image_refs = {}

    # Game setup
    matches = ["Fewa", "Fewa", "Flask", "Flask", "Computer", "Computer", "Pupcat", "Pupcat",
               "Milo", "Milo", "Teresa", "Teresa"]
    ...
    def check_match():
        ...
        if v1 == v2:
            ...
            if len(matched) == len(matches):
                ...
                show_end_screen(canvas, digit_images, remaining_time, "normal")
        else:
    ...
    # Card positions
    positions = [(50,218), (270, 218), (490, 218), (710, 218), (930, 218), (1150, 218),
                 (50, 480), (270, 480), (490, 480), (710, 480), (930, 480), (1150, 480)]
    ...
    def on_play_again_click(event=None):
        close_popup()
        load_normal_mode(canvas, load_difficult_mode)

    def on_next_level_click(event=None):
        close_popup()
        if load_difficult_mode:
    ...
    try:
        label_img = Image.open("label/normalL.png").resize((237, 98))
        canvas.image_refs['normal_label'] = ImageTk.PhotoImage(label_img)
        canvas.create_image(1200, 90, image=canvas.image_refs['normal_label'], anchor=tk.CENTER)
    except Exception as e:
        print("Normal label failed:", e)
